...
function drawLine(from, to){
    ...
    srcPoint = new THREE.Vector3(from);
    destPoint = new THREE.Vector3(to); 
...
function getHeadInfo(results){
        ...
        /* EulerPlane in calcHead.js */
        const p1 = new Vector(landmarks[21]);            
        const p2 = new Vector(landmarks[251]);
        const p3 = new Vector(landmarks[397]);
        const p4 = new Vector(landmarks[172]);
        const p3mid = p3.lerp(p4, 0.5);
        const p1mid = p1.lerp(p2, 0.5);
        const position = p1mid.lerp(p3mid, 0.5);

        const camOrg = new Vector(0.5, 0.5, 1);             // temporarily defined camera position
        const faceSize = position.distance(p1, 3);          // size from position to a corner of the Euler plane
        const faceDistance = position.distance(camOrg, 3);  // distance from camOrg to position

        return{
            // value range : 0.0 ~ 1.0 
            // should multiply canvas width & height to .x & .y of position and faceSize
            position,
            faceSize
        }
    }
...
function fillFaceOutline(meshCanvas, myCtx, results, color){
    myCtx.save();    
    ...
    cW = meshCanvas.width; 
    cH = meshCanvas.height;

    if (results.multiFaceLandmarks) {
        for (const landmarks of results.multiFaceLandmarks) {
            myCtx.beginPath();
                const foLen = faceOutline.length;
                myCtx.moveTo(landmarks[faceOutline[0]].x*cW,landmarks[faceOutline[0]].y*cH);
                for ( i = 1 ; i < foLen ; i++ ) {
                    myCtx.lineTo(landmarks[faceOutline[i]].x*cW,landmarks[faceOutline[i]].y*cH);
                    console.log(faceOutline[i]);
                }
            myCtx.closePath();
            myCtx.stroke();
            myCtx.fill(); 
...
function fillFaceAll(meshCanvas, myCtx, results, color){
    myCtx.save();
    myCtx.fillStyle = color;
    myCtx.strokeStyle = color; 
    myCtx.lineWidth = 1;
    ...
    if (results.multiFaceLandmarks) {
        for (const landmarks of results.multiFaceLandmarks) {
            for ( const ft of faceTriangle){
                myCtx.beginPath();
                myCtx.moveTo(landmarks[ft[0]].x*cW,landmarks[ft[0]].y*cH);
                myCtx.lineTo(landmarks[ft[1]].x*cW,landmarks[ft[1]].y*cH);
                myCtx.lineTo(landmarks[ft[2]].x*cW,landmarks[ft[2]].y*cH);
                myCtx.closePath();
                myCtx.stroke();
                myCtx.fill();    
            }

            for ( const ft of rightEye){
                myCtx.beginPath();
                myCtx.moveTo(landmarks[ft[0]].x*cW,landmarks[ft[0]].y*cH);
                myCtx.lineTo(landmarks[ft[1]].x*cW,landmarks[ft[1]].y*cH);
                myCtx.lineTo(landmarks[ft[2]].x*cW,landmarks[ft[2]].y*cH);
